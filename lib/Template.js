...
class Template {
    ...
    generate(model) {
        ...
        if (!Array.isArray(target)) {
            ...
            if (!this._settings.splitOn) {
                const result = new TemplateResult(
                    Template.prepareExportPath(this._settings, null, null),
                    content,
                ...
                for (let index = 0; index < sections.length; index++) {
                    ...
                    const result = new TemplateResult(
                        Template.prepareExportPath(this._settings, fileName, null),
                        section,
                        this._settings.appendToExisting
                        );
        ...
        for (const item of target) {
            ...
            if (!this._settings.splitOn) {
                const result = new TemplateResult(
                    Template.prepareExportPath(this._settings, null, item),
                    content,
                    this._settings.appendToExisting
                    );
                ...
                for (let index = 0; index < sections.length; index++) {
                    ...
                    const result = new TemplateResult(
                        Template.prepareExportPath(this._settings, fileName, item),
                        section,
                        this._settings.appendToExisting
                        );
    ...
    static prepareExportPath(settings, fileName, item) {
