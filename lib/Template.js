...
class Template {
    ...
    load(directoryPath, fileName) {
        ...
        const settingsContent = fs.readFileSync(`${directoryPath}/${templateSettingsFile}`, { encoding: 'utf8' });
        const templateContent = fs.readFileSync(`${directoryPath}/${templateFile}`, { encoding: 'utf8' });

        if (fs.existsSync(`${directoryPath}/${scriptFile}`))
            this._script = require(resolve(`${directoryPath}/${scriptFile}`));
